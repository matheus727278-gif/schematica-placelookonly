...
public class ConfigurationHandler {
    ...
    // MODIFICATION: new option - only place the block the player is looking at.
    public static final boolean PLACE_LOOK_ONLY_DEFAULT = false;
    ...
    // MODIFICATION: new option - only place the block the player is looking at.
    public static boolean placeLookOnly = PLACE_LOOK_ONLY_DEFAULT;
    ...
    // MODIFICATION: new option - only place the block the player is looking at.
    public static Property propPlaceLookOnly = null;
    ...
    private static void loadConfigurationPrinter() {
        ...
        // MODIFICATION: new option - only place the block the player is looking at.
        // Uses configuration.get(...) directly with raw strings so we don't have to
        // touch Names.java as well.
        propPlaceLookOnly = configuration.get(
                Names.Config.Category.PRINTER,
                "placeLookOnly",
                PLACE_LOOK_ONLY_DEFAULT,
                "Only place the block the player is currently looking at (aimed at with the crosshair), instead of scanning every position in range. Lets isolated/floating blocks (no adjacent support needed) be placed too."
        );
        propPlaceLookOnly.setLanguageKey(Names.Config.LANG_PREFIX + ".placeLookOnly");
        placeLookOnly = propPlaceLookOnly.getBoolean(PLACE_LOOK_ONLY_DEFAULT);
    }
