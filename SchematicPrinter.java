...
public class SchematicPrinter {
    ...
    public boolean print(final WorldClient world, final EntityPlayerSP player) {
        // MODIFICATION: if enabled, only ever try to place the single block the
        // player is currently looking at, instead of scanning every position in
        // range. This lets isolated / floating blocks (no adjacent support) be
        // placed too, since we never need to find a "solid neighbour" to aim at.
        if (ConfigurationHandler.placeLookOnly) {
            return printLookedAt(world, player);
        }
    ...
    /**
     * MODIFICATION: marches a ray out from the player's eyes along their look
     * vector (in 0.05 block steps, up to the normal block reach distance) and
     * attempts to place the FIRST schematic block along that ray whose state
     * doesn't already match the real world. Everything else (item swapping,
     * placeAdjacent handling, sneaking sync, timeouts, NBT sync...) reuses the
     * exact same placeBlock(...) pipeline as the original area-scan printer, so
     * all your other printer.* config options keep working normally.
     */
    private boolean printLookedAt(final WorldClient world, final EntityPlayerSP player) {
        final int slot = player.inventory.currentItem;
        final boolean isSneaking = player.isSneaking();

        syncSneaking(player, true);

        final double reach = this.minecraft.playerController.getBlockReachDistance() - 0.1;
        final Vec3d eyePos = player.getPositionEyes(1.0f);
        final Vec3d look = player.getLook(1.0f);

        final double step = 0.05;
        final int steps = (int) Math.ceil(reach / step);

        BlockPos lastChecked = null;
        for (int i = 0; i <= steps; i++) {
            final double dist = i * step;
            final Vec3d samplePoint = eyePos.addVector(look.x * dist, look.y * dist, look.z * dist);
            final BlockPos worldPos = new BlockPos(samplePoint);

            if (worldPos.equals(lastChecked)) {
                continue;
            }
            lastChecked = worldPos;

            final int sx = worldPos.getX() - this.schematic.position.x;
            final int sy = worldPos.getY() - this.schematic.position.y;
            final int sz = worldPos.getZ() - this.schematic.position.z;

            if (sx < 0 || sy < 0 || sz < 0
                    || sx >= this.schematic.getWidth()
                    || sy >= this.schematic.getHeight()
                    || sz >= this.schematic.getLength()) {
                continue;
            }

            final BlockPos localPos = new BlockPos(sx, sy, sz);

            switch (schematic.layerMode) {
            case ALL:
                break;
            case SINGLE_LAYER:
                if (sy != schematic.renderingLayer) {
                    continue;
                }
                break;
            case ALL_BELOW:
            ...
            try {
                if (placeBlock(world, player, localPos)) {
                    return syncSlotAndSneaking(player, slot, isSneaking, true);
                }
            } catch (final Exception e) {
                Reference.logger.error("Could not place block!", e);
